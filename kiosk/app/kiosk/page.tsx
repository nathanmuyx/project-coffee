...
export default function KioskPage() {
  ...
  if (flowState === "success") {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-slate-900 p-8">
        ...
        <p className="text-xl text-slate-400 mb-6">Your chip number is</p>
        <span className="text-7xl font-extrabold text-indigo-400 mb-6">
          #{chipNumber}
        </span>
        {paymentMethod === "cash" && (
          <p className="text-slate-400">Please pay at the counter. This screen will reset shortly.</p>
        )}
        {paymentMethod === "gcash" && (
          <p className="text-emerald-400 font-semibold">GCash payment noted. This screen will reset shortly.</p>
        )}
      </div>
    );
  }
  ...
  if (flowState === "confirming") {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-slate-900 p-8">
        <h2 className="text-2xl font-bold mb-8">Confirm Your Order</h2>
        <div className="bg-slate-800 rounded-2xl p-8 border border-slate-700 w-full max-w-sm text-center">
          ...
          {paymentMethod === "gcash" && (
            <div className="mb-4">
              <p className="text-sm text-slate-400 mb-3">Scan to pay with GCash</p>
                ...
                <img
                  ...
                  onError={(e) => {
                    ...
                    (e.target as HTMLImageElement).parentElement!.innerHTML =
                      '<div class="w-48 h-48 flex items-center justify-center text-slate-400 text-sm">QR placeholder<br/>Add gcash-qr.png to /public</div>';
                  }}
                />
              </div>
            </div>
          ...
          <div className="space-y-2">
            {cart.map((c) => (
              <div key={c.menuItem.id} className="flex justify-between text-sm text-slate-300">
                <span>{c.quantity}x {c.menuItem.name}</span>
                <span>{formatCurrency(c.menuItem.price * c.quantity)}</span>
              </div>
            ))}
          </div>
        </div>
        <div className="flex gap-3 mt-8 w-full max-w-sm">
          <button
            ...
            className="flex-1 py-3 rounded-xl bg-slate-800 hover:bg-slate-700 font-semibold transition-colors"
          >
            Back
          </button>
          <button
            onClick={submitOrder}
            disabled={submitting}
            className="flex-1 py-3 rounded-xl bg-indigo-500 hover:bg-indigo-400 disabled:opacity-50 font-semibold transition-colors"
          >
  ...
  if (flowState === "payment") {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-slate-900 p-8">
        ...
        <p className="text-slate-400 mb-8">{formatCurrency(total)}</p>
        ...
        <div className="flex gap-4 w-full max-w-md">
          <button
            ...
            className="flex-1 flex flex-col items-center gap-4 p-8 rounded-2xl bg-slate-800 border-2 border-slate-700 hover:border-emerald-500 transition-all active:scale-95"
          >
            ...
            <span className="text-sm text-slate-400">Pay at the counter</span>
          </button>
          ...
          <button
            ...
            className="flex-1 flex flex-col items-center gap-4 p-8 rounded-2xl bg-slate-800 border-2 border-slate-700 hover:border-blue-500 transition-all active:scale-95"
          >
            ...
            <span className="text-sm text-slate-400">Scan QR to pay</span>
          </button>
        </div>
      </div>
    );
    ...
    return (
      <div className="flex flex-col h-screen bg-slate-900 p-6">
        <button
          ...
          className="self-start mb-4 text-slate-400 hover:text-slate-200 transition-colors flex items-center gap-2"
        >
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
          </svg>
        ...
        <div className="border-t border-slate-700 pt-4 mt-4">
          ...
          <button
            ...
            className="w-full py-4 rounded-xl bg-indigo-500 hover:bg-indigo-400 disabled:bg-slate-700 font-semibold text-lg transition-colors"
          >
            Continue
          </button>
        </div>
    ...
    <div className="flex h-screen bg-slate-900">
      <div className="flex-1 min-w-0 flex flex-col">
        <div className="p-4 pb-0">
          <p className="text-slate-400 text-sm">Tap items to add to your order</p>
        </div>
        <MenuGrid
          drinks={displayMenu}
          onAddItem={addItem}
